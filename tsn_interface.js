...
function ifFireOnline(callback) {
    chrome.extension.sendMessage( {cmd: 'get_fire_status'}, function(data) {
...
function enableTsn() {
    ...
    forTabsWithUrl(
        TSNURL,
        function(tab) {
            messageToTab(tab.id, {cmd:'status', status:true}, function(data) {
...
function onUpdateTsn(id, status, tab) {
    ...
    if (tab.status=='complete' && tab.url.contains(TSNURL)) {
        messageToTab(id, {cmd: 'getCheckBoxes'}, function(cbData) {
            ...
            getFireRecords('rent', function(records) {
                ...
                messageToTab(id, {cmd: 'setCheckBoxes', data: cbData.checkBoxes}, function() {
                });
            }.bind(this));
        }.bind(this));
    }
